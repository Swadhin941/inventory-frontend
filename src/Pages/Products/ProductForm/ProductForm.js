...
const ProductForm = ({ product, onClose }) => {
  ...
  {({ getFieldValue }) => {
    ...
    if (margin === undefined || profit === undefined) return null;

    return (
      <div className={`margin-box ${isLoss ? "loss" : ""}`}>
        <div>
          <p className="margin-label">CALCULATED MARGIN</p>
          <h2 className="margin-value">{margin}</h2>
        </div>

        <div className="profit-box">
          <p className="profit-label">Profit per unit</p>
          <h3 className="profit-value">QAR {profit}</h3>
        </div>
      </div>
    );
  }}
